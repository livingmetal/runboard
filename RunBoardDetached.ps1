...
function RunNow($c,$p){
    ...
    if(-not(Test-Path $target -PathType Leaf)){Bad "Target exe not found: $target";return}
    ...
    if($others.Count -gt 0){$r=$others[0];$msg="Current time overlaps another user's reservation.`r`n`r`nUser: $($r.user)`r`nTime: $(FTime $r.startTime) ~ $(FTime $r.endTime)`r`nPurpose: $($r.purpose)`r`n`r`nRun anyway?";if((Popup "RunBoard reservation warning" $msg $true) -ne "Yes"){Warn "Run cancelled by user.";return}}
    $now=Get-Date
